...
- (void)validate
{
    ...
    double minRowCount = self.minRowCount;
    double minColumnCount = self.minColumnCount;
    ...
    while (!self.resolved && !self.unresolvable) {
        ...
        if (cellWidth <= minCellWidth) {
            ...
            if (cellHeight <= minCellHeight) {
                ...
                if (((rowCount ? rowCount - 1 : 0)  * columnCount >= self.minimumNumberOfCells) &&
...
#define OFFSET 8 // deafult number for view
- (CGRect)frameOfCellAtRow:(NSUInteger)row inColumn:(NSUInteger)column
{
    CGRect frame = CGRectMake(0 + OFFSET, 0 + OFFSET, self.cellSize.width - 2 * OFFSET, self.cellSize.height - 2 * OFFSET);
